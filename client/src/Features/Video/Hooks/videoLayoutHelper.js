...
//create array of layout candidates
const layoutCandidates = Array.from({length:9}).map((value, index) => {
  const count = index + 1;
  const mid = Math.ceil(count/2);
  const candidates = Array.from({length: mid}).map((val, ind) => {
    const row = ind + 1;
...
const minCellW = 256;
const minCellH = minCellW/aspectRatio;
const cellOffset = 5;
const maxCount = 9;

//function to find max rows and columns
const maxRowCol = (width, height) => ({
  maxColumns : Math.max(1, Math.floor(width/(minCellW + cellOffset * 2))),
  maxRows : Math.max(1, Math.floor(height/(minCellH + cellOffset * 2)))
});

export const maxViewportVideoCounts = (width, height) => {
  const {maxRows, maxColumns} = maxRowCol(width, height);
  return maxRows * maxColumns
};

export const getVideoLayout = (rootWidth, rootHeight, count) => {
  if (count > maxCount || count === 0) {
    return []
  };
  let { maxRows, maxColumns } = maxRowCol(rootWidth, rootHeight);
  ...
  const preferredLayout = layoutOfCount.map((el) => {
    const { column, row } = el;
    ...
    return {
      cellWidth, cellHeight, cellArea: cellWidth * cellHeight, column, row
    }
  }).reduce((prev, curr) => {
